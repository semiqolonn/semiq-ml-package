...
class BaselineModel:
    ...
    def __init__(self, task_type="classification", metric=None, random_state=42):
        ...
        )  # Now holds a dict of {name: model_instance}
        ...
        self._metric_functions = {
            ...
            "log_loss": lambda y_true, y_pred: (
                -np.mean(np.log(y_pred + 1e-15))
    ...
    def _set_metric_and_direction(self, metric):
        ...
        self.maximize_metric = (
            True  # Default to maximizing (e.g., accuracy, R2, ROC AUC)
        )
        ...
        if self.task_type == "classification":
            ...
            if self.metric not in [
                ...
                raise ValueError(
                    f"Invalid metric '{self.metric}' for regression. Choose from 'neg_root_mean_squared_error', 'r2', 'neg_mean_absolute_error'."
                )
        ...
        if self.task_type == "classification":
            return {
                "Logistic Regression": LogisticRegression(
                    random_state=self.random_state, solver="liblinear", max_iter=1000
                ),
                # SVC probability is expensive; enable only if ROC AUC is desired metric
                "SVC": SVC(
                    random_state=self.random_state,
                    probability=(self.metric in ("roc_auc", "auc")),
                ),
                "KNN": KNeighborsClassifier(),
                "Decision Tree": DecisionTreeClassifier(random_state=self.random_state),
                "Random Forest": RandomForestClassifier(
                    random_state=self.random_state
                ),  # n_estimators handled by tuning
                "LGBM": LGBMClassifier(
                    random_state=self.random_state, verbosity=-1
                ),  # n_estimators, lr handled by tuning
                ...
                "CatBoost": CatBoostClassifier(
                    random_state=self.random_state, silent=True
                ),  # iterations, lr handled by tuning
            }
        else:  # Regression
            return {
        ...
        if self.task_type == "classification":
            if self.metric in ("roc_auc", "auc"):
                has_proba = (
                    ...
                    hasattr(model, "probability")
                    and getattr(model, "probability") is True
                )
                ...
                if has_proba:
                    ...
                    if is_multiclass:
                        logger.warning(
                            "ROC AUC for multi-class is complex. Using f1_weighted instead."
                        )
                    ...
                    logger.warning(
                        f"Model {model.__class__.__name__} does not support predict_proba or `probability=True` not set. Falling back to accuracy for ROC AUC metric."
                    )
                    y_pred = model.predict(X_val)
    ...
    def get_model(self, model_name):
        ...
        if model_name in self.results and self.results[model_name]["model"] is not None:
            ...
            raise ValueError(
                f"Model '{model_name}' not found or failed to train. Available models: {list(self.results.keys())}"
            )
    ...
    def roc_curves(self, X, y):
        ...
        # Keep original data for CatBoost and preprocess for other models
        original_X = X.copy() if isinstance(X, pd.DataFrame) else None
        if isinstance(X, pd.DataFrame):
            processed_X = self._preprocess_features(X, fit=False)
        else:
        ...
        for name, model_info in self.results.items():
            ...
            try:
                # Use appropriate data for the model type
                is_catboost = model_info.get("is_catboost", False)
                eval_X = original_X if is_catboost and original_X is not None else processed_X
